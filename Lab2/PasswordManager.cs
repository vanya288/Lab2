...
namespace Lab2
{
    public class PasswordManager
    {
        ...
        /// <summary>
        /// Creates the password hash SHA512
        /// </summary>
        /// <param name="_password">Password string</param>
        /// <returns>SHA512 hash in base64 string</returns>
        public string CreatePasswordHashSHA512(string _password)
        {
            string salt = this.GenerateSaltString();

            return this.GetPasswordHashSHA512(_password, salt);
        }


        /// <summary>
        /// Creates the password hash HMAC SHA512
        /// </summary>
        /// <param name="_password">Password string</param>
        /// <returns>HMAC SHA512 hash in base64 string</returns>
        public string CreatePasswordHashHMACSHA512(string _password)
        {
            string salt = this.GenerateSaltString();

            return this.GetPasswordHashHMACSHA512(_password, salt);
        }
        ...
        /// <param name="_secretText">Text to be encrypted</param>
        /// <returns>HMAC SHA512 hash in base64 string</returns>
        public string CalculateHMACSHA512(string _password, string _salt)
        {
            string salt = _salt ?? "";
            var encoding = new ASCIIEncoding();
        ...
        public virtual string GenerateSaltString(int _saltLength = 64)
        {
        ...
        public byte[] GenerateSaltBytes(int _saltLength = 64)
        {
            ...
            if (_saltLength <= 0)
            {
                throw new ArgumentOutOfRangeException($"{nameof(_saltLength)} must be more than 0.");
            }

            rngCryptoServiceProvider = new RNGCryptoServiceProvider();
            randomBytes = new byte[_saltLength];
